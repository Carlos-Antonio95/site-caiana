...
{
    // Pega todos os produtos que têm preço final diferente do preço normal
    $promoProducts = Products::where('status', 'ativo')
        ->get()
        ->filter(fn($p) => $p->final_price < $p->price); // filtra só os produtos em promoção
